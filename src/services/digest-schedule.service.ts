 ...
 * Cadencia Redis en redis.service (`digest:next_regular:*`).
 *
 * Ventana: Europe/Madrid. Hard floor NOTIF_HARD_* (default 7–23).
 * Start UI: 07–12 · End UI: 19–23 · Intervalo: 1–4 h (default 2).
 */
import { prisma } from '../db/prisma';
import { cacheDel, cacheGetJson, cacheSetJson, redisAvailable } from './redis.service';

const PREFS_CACHE_TTL_SEC = 5 * 60;
const prefsMemory = new Map<string, { value: DigestPrefs; expiresAt: number }>();
...
/** Partes Europe/Madrid (no depende de TZ del contenedor). */
...
export async function invalidateDigestPrefsCache(telegramId: string): Promise<void> {
  prefsMemory.delete(prefsKey(telegramId));
  await cacheDel(prefsKey(telegramId));
}

export async function loadDigestPrefs(telegramId: string): Promise<DigestPrefs> {
  const mem = prefsMemory.get(prefsKey(telegramId));
  if (mem && mem.expiresAt > Date.now()) return mem.value;

  if (redisAvailable()) {
    const cached = await cacheGetJson<DigestPrefs>(prefsKey(telegramId));
    if (cached) {
      const prefs = clampSchedulePrefs(cached);
      prefsMemory.set(prefsKey(telegramId), {
        value: prefs,
        expiresAt: Date.now() + PREFS_CACHE_TTL_SEC * 1000,
      });
      return prefs;
    }
  }
  ...
  try {
    ...
    await cacheSetJson(prefsKey(telegramId), prefs, PREFS_CACHE_TTL_SEC);
    return prefs;
  } catch {
...
export async function saveDigestPrefs(
  ...
  await cacheSetJson(prefsKey(telegramId), clamped, PREFS_CACHE_TTL_SEC);
  return clamped;
}
