...
async function renderPanel(ctx: Context, telegramId: string, edit: boolean) {
  ...
  const text = [
    ...
    dirty
      ...
      : '',
    `<i>Pulsa Listo para guardar. Las alertas VIP (y el lote rápido tras Aplicar filtros) solo se envían en esta ventana.</i>`,
  ]
    .filter(Boolean)
    .join('\n');
...
async function renderHoursStart(ctx: Context, draft: HorarioDraft, edit: boolean) {
  ...
  const text = [
    ...
    `<i>Por defecto 08:00. Elige 07:00–12:00 y pulsa Listo.</i>`,
  ].join('\n');
...
async function renderHoursEnd(ctx: Context, draft: HorarioDraft, edit: boolean) {
  ...
  const rows = hourChoiceButtons(draft.endHour, ends, 'horario_end_');
  rows.push([
  ...
  const text = [
    ...
    ``,
    `Ventana actual: <b>${formatHourRange(draft.startHour, draft.endHour)}</b>`,
    `<i>Por defecto 21:00 (exclusiva). Elige 19:00–23:00 y pulsa Listo.</i>`,
  ].join('\n');
...
export function setupHorarioMenu(bot: Telegraf): void {
  ...
  bot.action(/^horario_start_(\d+)$/, async (ctx) => {
    ...
    if (!SCHEDULE_START_HOURS.includes(h as (typeof SCHEDULE_START_HOURS)[number])) {
      await ctx.answerCbQuery();
      return;
    }
    const telegramId = String(ctx.from!.id);
  ...
  bot.action('horario_save', async (ctx) => {
    ...
    try {
      ...
      const { getNextRegularAtMs, setNextRegularAtMs } = await import('../services/redis.service');
